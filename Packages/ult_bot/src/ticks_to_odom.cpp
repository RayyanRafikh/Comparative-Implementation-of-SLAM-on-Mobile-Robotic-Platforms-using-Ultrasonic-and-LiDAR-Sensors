...
const double TICKS_PER_REVOLUTION = 100; // For reference purposes.
const double WHEEL_RADIUS = 0.03225; // Wheel radius in meters
const double WHEEL_BASE = 0.17; // Center of left tire to center of right tire
const double TICKS_PER_METER = 494; // Original was 2800
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subForRightCounts = node.subscribe("r_ticks", 100, Calc_Right, ros::TransportHints().tcpNoDelay());
  ros::Subscriber subForLeftCounts = node.subscribe("l_ticks", 100, Calc_Left, ros::TransportHints().tcpNoDelay());
  ...
  while(ros::ok()) {
    ...
    transform.setOrigin(tf::Vector3(odomNew.pose.pose.position.x, odomNew.pose.pose.position.y,odomNew.pose.pose.position.z));
    q2.setRPY(0, 0, odomNew.pose.pose.orientation.z);
